...
The spec-decoding win->loss crossover is B = a * B*, where B* = W /
...
weights grow ~H^2 (attention/MLP projections) while KV per token grows ~H
(kv_heads * head_dim, held fixed under GQA). So W / kv_per_token ~ H, B* rises
with scale, and the spec crossover rides along.

"Speculative decoding inverts under batching on generic traffic" is the most
scale-sensitive claim in this repo: draft cost vs target cost is exactly the
ratio expected to move with model size. The cost model predicts where the
crossover lands at every size before a GPU runs; bench/spec_batched_study.py
measures it per model.
...
# across sizes; whether that holds is itself a prediction the GPU checks.
...
def main():
    ...
    print(f"""
  B* rises with scale (weights ~H^2, KV ~H, so W/KV ~ H), so every crossover
  moves right. But the generic-prose crossover stays in the single digits
  ({gen}), far below any real serving batch (16-64), so "spec decoding is a net
  loss on batched generic traffic" holds at 1.5B and 3B, not just 0.5B. The
  grounded win extends further ({grd}).
