...
and head_dim, mapping to signed `bits`-bit ints and back. Simulates storing the
KV cache at `bits` bits instead of fp32; cache memory drops by 32/bits, and the
audit asks what that costs in output quality (how many generated tokens drift
from the fp32 result).
"""
...
def _quantize(x: torch.Tensor, qmax: int) -> torch.Tensor:
    ...
    # because heads differ in magnitude; per-head is the baseline.
...
def quantize_cache(cache, bits: int):
    ...
    version, in place. Simulates low-bit KV storage."""
    if not bits:
        return
    qmax = 2 ** (bits - 1) - 1
